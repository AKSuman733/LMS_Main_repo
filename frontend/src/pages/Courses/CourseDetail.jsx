...
const getCelebrityImage = (name, celebritiesList = []) => {
  if (!name) return null;
  
  // First check if the celebrity exists in the dynamic list
  const celeb = celebritiesList.find(c => c.name === name);
  if (celeb && celeb.image_url) {
    if (celeb.image_url.startsWith('/uploads')) {
      return `http://localhost:5001${celeb.image_url}`;
    }
    return celeb.image_url;
  }
...
>>>>>>> 9e1de81cd6878b26aed245c1ff99ddd4ff053383
  ...
  const handleSuccessEnrollment = async () => {
    try {
      ...
      toast.error(err.response?.data?.error || 'Enrollment failed after payment');
    }
  };
...
>>>>>>> 9e1de81cd6878b26aed245c1ff99ddd4ff053383
      ...
      if (course && course.price && parseFloat(course.price) > 0) {
        ...
        setClientSecret(''); // reset previous intent
        
        const response = await axios.post(
          'http://localhost:5001/api/courses/create-payment-intent',
          { course_id: id },
          { headers: { Authorization: `Bearer ${token}` } }
        );
...
>>>>>>> 9e1de81cd6878b26aed245c1ff99ddd4ff053383
                  ...
                  onChange={(e) => {
                    ...
                    if (val) {
                      localStorage.setItem(`course_${id}_celebrity`, val);
                    } else {
                      localStorage.removeItem(`course_${id}_celebrity`);
                    }
                  }}
                >
...
>>>>>>> 9e1de81cd6878b26aed245c1ff99ddd4ff053383
                      style={{
                        ...
                        boxShadow: (selectedCelebrity.toLowerCase().includes('shahrukh') || selectedCelebrity.toLowerCase().includes('srk')) ? '0 0 10px #fbbf24' : 'none'
                      }}
                    />
                  )}
                  <span style={{fontSize: '0.85rem', letterSpacing: '0.5px'}}>Presented by {selectedCelebrity}</span>
                </div>
              )}
