...
const CourseDetails = () => {
    const { id } = useParams();
    const { user, selectedInstructor, instructors } = useContext(AuthContext);
    const localInstructorId = parseInt(localStorage.getItem(`course_instructor_${user?.id}_${id}`));
    const courseSelectedInstructor = (instructors && localInstructorId)
        ? (instructors.find(i => i.id === localInstructorId) || selectedInstructor)
        : selectedInstructor;
    ...
    const fetchDetails = async () => {
        ...
        try {
            ...
            console.error(err);
            setError("Failed to load course details. Please check your network and try again.");
        } finally {
            setLoading(false);
        }
    };
    ...
    if (loading) return (
        <div className="premium-page-loader skeleton-pulse">
            ...
            <p className="loader-text">Loading... Syncing Course Data</p>
        </div>
    );
    if (error) return (
        <div className="premium-error-state course-details-error-state">
            ...
            <div className="error-actions">
                <button className="error-retry-btn" onClick={fetchDetails}>Retry</button>
    ...
    if (!course) return (
        <div className="premium-empty-state course-details-empty-state">
            ...
            <p>We couldn't locate the course you are looking for. It may have been archived or removed.</p>
    ...
    const displayInstructorName = courseSelectedInstructor ? courseSelectedInstructor.name : "No Celebrity Selected";
    const displayInstructorImage = courseSelectedInstructor ? courseSelectedInstructor.image : null;
    const displayInstructorBio = courseSelectedInstructor ? courseSelectedInstructor.bio : "You have not selected a celebrity mentor yet. Go to your dashboard or start the course player to pick your celebrity guide!";
            ...
            <div className="details-container-premium">
                ...
                <div className="course-hero-modern">
                    ...
                    <div className="hero-visual-card">
                        <img src={normalizeUrl(course.image)} alt={course.title} className="hero-img" onError={(e) => e.target.src = "https://images.unsplash.com/photo-1673515334386-2b24073bb22f?w=800"} />
                        <div className="instructor-overlay-card">
                            <img src={displayInstructorImage ? normalizeUrl(displayInstructorImage) : "https://plus.unsplash.com/premium_photo-1677252438411-9a930d7a5168?w=50"} alt={displayInstructorName} />
                        ...
                        <section className="details-section">
                            ...
                            <div className="flat-curriculum-list">
                                ...
                                {(!course.curriculum || (Array.isArray(course.curriculum) && course.curriculum.length === 0)) && (
                                    <p className="no-curriculum-text course-details-no-curriculum-text">Curriculum details coming soon.</p>
                                )}
                            </div>
                        </section>

                        <section className="details-section instructor-bio-section">
                            <div className="course-details-instructor-bio-card">
                                <img src={displayInstructorImage ? normalizeUrl(displayInstructorImage) : "https://plus.unsplash.com/premium_photo-1677252438411-9a930d7a5168?w=100"} className="course-details-instructor-img" alt={displayInstructorName} />
                                <div>
                                    <h3 className="course-details-instructor-title">Meet Your Instructor</h3>
                                    <h4 className="course-details-instructor-name">{displayInstructorName}</h4>
                                    <p className="course-details-instructor-description">
                            ...
                            <div className="reviews-container">
                                {user ? (
                                    isEnrolled ? (
                                        ...
                                        <div className="course-details-enroll-to-review-msg">
                                            <p className="course-details-enroll-to-review-text">Only enrolled students can share their experience. Start learning today to post your review!</p>
                                        </div>
                                    )
                                ) : (
                                    <div className="course-details-login-to-review-msg">
                                        <p className="course-details-login-to-review-text">Please <Link to="/login" className="course-details-login-link">login</Link> to join the discussion.</p>
                                    </div>
                                )}
