...
export default function ManageStudents() {
  ...
  const deleteSelected = () => {
    ...
    setSelectedRows([]);
    showToast(`${selectedRows.length} students deleted`, "warning");
  };

  const archiveSelected = () => {
    showToast(`${selectedRows.length} students archived`, "warning");
    setSelectedRows([]);
  };

  const exportSelected = () => showToast("PDF exported successfully", "success");
  const exportStudent = (student) => { setActiveMenu(null); showToast(`${student.name}'s record exported`, "success"); };
...
<Sidebar role="admin" />
      ...
      <main className="flex-1 md:ml-64 p-4 md:p-8 overflow-x-auto">
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: 24, marginBottom: 32 }}>
          {loading ? (
            ...
            [
              ...
              <motion.div key={i} whileHover={{ y: -4, scale: 1.01 }} style={{ padding: 24, borderRadius: 32, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)", backdropFilter: "blur(20px)", position: "relative", overflow: "hidden" }}>
        ...
        {/* TABLE */}
        <div style={{ borderRadius: 32, border: "1px solid rgba(255,255,255,0.08)", background: "rgba(255,255,255,0.04)", backdropFilter: "blur(16px)", overflow: "auto" , width: "100%" }}>
                    ...
                    {activeMenu === student.id && (
                      ...
                      >
                        {[
                          ...
                          { label: "Edit Student", icon: "✏️", action: () => { showToast("Edit coming soon", "warning"); setActiveMenu(null); } },
