...
export function MetricCard({
  ...
  let RenderIcon = iconMap[icon as keyof typeof iconMap] || IconActivity;
  ...
  if (isSystemHealth) {
    ...
    if (!isNaN(numValue)) {
      if (numValue >= 99) {
        RenderIcon = IconCircleCheck;
        iconColor = colors.success;
      } else if (numValue < 90) {
        RenderIcon = IconAlertOctagon;
        iconColor = colors.error;
      } else if (numValue < 95) {
        RenderIcon = IconAlertTriangle;
  ...
  return (
    <div
      ...
      style={{
        ...
        background: `color-mix(in srgb, ${bgColor} 10%, ${colors.surface})`,
