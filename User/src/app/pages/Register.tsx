...
export function Register() {
  ...
  const strengthColors = ['#EF4444', '#FF8C42', '#EAB308', '#00C97B'];
  ...
  return (
    <div className="min-h-screen flex bg-[#0A0F1E] text-white">
      ...
      <div className="flex-grow flex-1 bg-[#0A0F1E] flex items-center justify-center p-6 sm:p-12">
        {isSubmitted ? (
          /* SUCCESS STATE */
          <div className="w-full max-w-[420px] flex flex-col items-center text-center py-8">
            <div className="w-[80px] h-[80px] rounded-full bg-[#FF6B2B]/20 flex items-center justify-center text-white mb-6 border border-[#FF6B2B]/30 shadow-lg">
              <Check size={40} strokeWidth={3} className="text-[#FF6B2B]" />
            </div>
            <h2 className="text-[28px] font-bold text-white mb-2 leading-tight">
              Account created!
            </h2>
            <p className="text-[15px] text-[#9CA3AF] mb-8 max-w-[320px]">
              Check your email to verify your account.
            </p>
            <button
              onClick={() => navigate('/dashboard')}
              className="w-full h-[48px] bg-[#FF6B2B] text-white font-bold text-[15px] rounded-[8px] flex items-center justify-center gap-2 hover:bg-[#FF8C42] active:scale-[0.97] transition-all cursor-pointer border-none shadow-md"
            >
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex gap-[10px] w-full mb-5">
                ...
                <button
                  ...
                  className={`flex-1 flex flex-col items-start p-4 rounded-[8px] border text-left transition-all cursor-pointer ${
                    role === 'student'
                ...
                >
                  <GraduationCap 
                    size={24} 
                    className={`mb-2 transition-colors ${role === 'student' ? 'text-white' : 'text-[#9CA3AF]'}`} 
                  />
                  <span className={`text-[14px] font-bold block mb-0.5 ${role === 'student' ? 'text-white' : 'text-[#9CA3AF]'}`}>
                    I'm a Student
                  </span>
                  <span className="text-[12px] text-[#9CA3AF]">Learn new skills</span>
                </button>
                ...
                >
                  <Presentation 
                    size={24} 
                    className={`mb-2 transition-colors ${role === 'instructor' ? 'text-white' : 'text-[#9CA3AF]'}`} 
                  />
                  <span className={`text-[14px] font-bold block mb-0.5 ${role === 'instructor' ? 'text-white' : 'text-[#9CA3AF]'}`}>
                    I'm an Instructor
                  </span>
                  <span className="text-[12px] text-[#9CA3AF]">Share knowledge</span>
                </button>
              </div>
                ...
                {formData.password && (
                  <div className="mt-2.5">
                    <div className="flex justify-between items-center mb-1.5">
                      <span className="text-[11px] text-[#9CA3AF] font-semibold uppercase tracking-wider">Security Strength</span>
                      <span className="text-[12px] font-bold" style={{ color: strengthColors[strength - 1] }}>
                        {strengthLabels[strength - 1]}
                      </span>
                    </div>
                    <div className="flex gap-[6px] h-[4px] w-full rounded-[8px] overflow-hidden bg-[#1E2D45]">
                      {[1, 2, 3, 4].map((index) => (
                        <div
                          key={index}
              ...
              <div className="flex flex-col w-full">
                ...
                <div className={`relative flex items-center rounded-[8px] border bg-[#111827] transition-all focus-within:shadow-[0_0_10px_rgba(255,107,43,0.15)] ${
                  passwordsMatch
                    ? 'border-[#1E2D45] focus-within:border-[#FF6B2B]'
              ...
              <div className="flex items-start gap-2.5 pt-1">
                <div className="relative flex items-center h-5">
                  <input
                    ...
                    className="w-4 h-4 rounded border-[#1E2D45] text-[#FF6B2B] focus:ring-0 focus:ring-offset-0 bg-[#111827] cursor-pointer accent-[#FF6B2B]"
                    required
                  />
                </div>
                <label htmlFor="terms" className="text-[14px] text-[#9CA3AF] leading-normal select-none">
                  I agree to the{' '}
