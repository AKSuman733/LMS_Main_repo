...
exports.enrollInCourse = async (req, res) => {
  ...
  try {
    ...
    // Notify user
    await notificationUtil.createNotification(user_id, 'Enrolled Successfully', `You have successfully enrolled in ${courseTitle}.`, 'success');
    
    // Notify admins
    const userInfo = await db.query('SELECT name FROM users WHERE id = $1', [user_id]);
    const userName = userInfo.rows[0]?.name || 'A student';
    await notificationUtil.notifyAllAdmins('New Enrollment', `${userName} enrolled in ${courseTitle}.`, 'info');

    // If it's a paid course, notify about payment as well
    if (parseFloat(coursePrice) > 0) {
      await notificationUtil.notifyAllAdmins('Payment Received', `Payment of ₹${coursePrice} received from ${userName} for ${courseTitle}.`, 'success');
...
exports.getUserDashboardStats = async (req, res) => {
  const user_id = req.user.id;
  try {
    // 1. Skill Distribution based on enrolled categories
    ...
    let skillData = [];
    const totalCourses = categoriesQuery.rows.reduce((sum, row) => sum + row.count, 0);
    if (totalCourses > 0) {
      skillData = categoriesQuery.rows.map(row => ({
        name: row.name,
        value: Math.round((row.count / totalCourses) * 100)
      }));
    }

    // 2. Total completed lessons for knowledge points and total hours
    const totalCompletedQuery = await db.query(`
      SELECT COUNT(*) as count 
      FROM progress 
      WHERE user_id = $1 AND status = 'completed'
    `, [user_id]);
    
    const totalCompleted = parseInt(totalCompletedQuery.rows[0].count);
    const knowledgePoints = totalCompleted * 50;
    const hoursLearned = (totalCompleted * 0.5).toFixed(1);
    ...
    const dailyActivityQuery = await db.query(`
      ...
      SELECT 
        ...
        d.date as full_date,
        COUNT(p.lesson_id)::int as lessons_completed
      FROM last_7_days d
      LEFT JOIN progress p ON DATE(p.updated_at) = d.date AND p.user_id = $1 AND p.status = 'completed'
      GROUP BY d.date
      ORDER BY d.date
    `, [user_id]);

    const learningData = dailyActivityQuery.rows.map(row => ({
      name: row.name,
      hours: parseFloat((row.lessons_completed * 0.5).toFixed(1)),
      date: row.full_date
    }));
...
exports.exportCourses = async (req, res) => {
  try {
    const query = `SELECT id, title, category, level, price, duration, created_at FROM courses ORDER BY id`;
    const result = await db.query(query);
    
    const headers = ['ID', 'Title', 'Category', 'Level', 'Price', 'Duration', 'Created At'];
    ...
    res.status(500).json({ error: 'Error exporting courses' });
  }
};
...
exports.bulkDeleteCourses = async (req, res) => {
  const { ids } = req.body;
  try {
    await db.query('DELETE FROM courses WHERE id = ANY($1::int[])', [ids]);
...
exports.createCheckoutSession = async (req, res) => {
  ...
  try {
    ...
    if (!course.price || parseFloat(course.price) <= 0) {
      return res.status(400).json({ error: 'Course does not require payment' });
    }
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: 'payment',
      success_url: `${process.env.FRONTEND_URL}/student/course/${course_id}?success=true&session_id={CHECKOUT_SESSION_ID}`,
      cancel_url: `${process.env.FRONTEND_URL}/student/course/${course_id}?canceled=true`,
