...
exports.login = async (req, res) => {
  ...
  try {
    ...
    if (user.rows[0].otp) {
      if (password !== user.rows[0].otp) {
        return res.status(403).json({ error: 'For your first login after approval, please use the OTP sent to your email as your password.' });
      }
      // Clear OTP after successful use
      await db.query('UPDATE users SET otp = NULL WHERE id = $1', [user.rows[0].id]);
    } else {
      const isMatch = await bcrypt.compare(password, user.rows[0].password);
      if (!isMatch) {
        // Check temp password
        if (user.rows[0].temp_password && password === user.rows[0].temp_password) {
          // Allow login with temp password
        } else {
          return res.status(400).json({ error: 'Invalid credentials' });
        }
      }
...
exports.getPendingUsers = async (req, res) => {
  try {
    const users = await db.query('SELECT id, name, email, role, created_at FROM users WHERE is_approved = FALSE AND (is_archived = FALSE OR is_archived IS NULL) AND (is_deleted = FALSE OR is_deleted IS NULL)');
...
exports.getAdminStats = async (req, res) => {
  try {
    ...
    const pendingCount = await db.query("SELECT COUNT(*) FROM users WHERE is_approved = FALSE AND (is_archived = FALSE OR is_archived IS NULL) AND (is_deleted = FALSE OR is_deleted IS NULL)");
    ...
    const popularCoursesQuery = await db.query(`
      SELECT 
        c.id, c.title, c.category, c.thumbnail, c.price,
        COUNT(e.id)::int as enrollments_this_week
      FROM courses c
      JOIN enrollments e ON c.id = e.course_id
      WHERE e.enrolled_at >= NOW() - INTERVAL '7 days'
      GROUP BY c.id, c.title, c.category, c.thumbnail, c.price
      ORDER BY enrollments_this_week DESC
      LIMIT 5
    `);
    ...
    res.status(200).json({
      ...
      popularCourses: popularCoursesQuery.rows
    });
  } catch (error) {
    console.error(error);
...
exports.getAllStudents = async (req, res) => {
  ...
  let statusCondition = "u.role = 'student'";
  if (status === 'deleted') {
    statusCondition += " AND u.is_deleted = TRUE";
  } else if (status === 'archive') {
    statusCondition += " AND u.is_archived = TRUE AND (u.is_deleted = FALSE OR u.is_deleted IS NULL)";
  } else {
    // Default to active
    statusCondition += " AND (u.is_archived = FALSE OR u.is_archived IS NULL) AND (u.is_deleted = FALSE OR u.is_deleted IS NULL)";
  }
...
exports.exportUsers = async (req, res) => {
  try {
    const query = `SELECT id, name, email, role, created_at, is_approved FROM users WHERE (is_archived = FALSE OR is_archived IS NULL) AND (is_deleted = FALSE OR is_deleted IS NULL) ORDER BY id`;
    const result = await db.query(query);
    
    const headers = ['ID', 'Name', 'Email', 'Role', 'Joined At', 'Approved'];
...
exports.archiveUser = async (req, res) => {
  const { id } = req.params;
  try {
    await db.query('UPDATE users SET is_archived = TRUE WHERE id = $1', [id]);
...
exports.bulkArchiveUsers = async (req, res) => {
  const { ids } = req.body;
  try {
    await db.query('UPDATE users SET is_archived = TRUE WHERE id = ANY($1::int[])', [ids]);
    res.status(200).json({ message: 'Users archived successfully' });
  } catch (error) {
    console.error('Error archiving users:', error);
    res.status(500).json({ error: 'Error archiving users' });
  }
};
...
exports.bulkDeleteUsers = async (req, res) => {
  ...
  try {
    if (hardDelete) {
      ...
      await db.query('UPDATE users SET is_deleted = TRUE WHERE id = ANY($1::int[])', [ids]);
    }
...
exports.restoreUser = async (req, res) => {
  const { id } = req.params;
  try {
    await db.query('UPDATE users SET is_archived = FALSE, is_deleted = FALSE WHERE id = $1', [id]);
...
exports.bulkRestoreUsers = async (req, res) => {
  const { ids } = req.body;
  try {
    await db.query('UPDATE users SET is_archived = FALSE, is_deleted = FALSE WHERE id = ANY($1::int[])', [ids]);
    res.status(200).json({ message: 'Users restored successfully' });
  } catch (error) {
    console.error('Error restoring users:', error);
    res.status(500).json({ error: 'Error restoring users' });
  }
};
