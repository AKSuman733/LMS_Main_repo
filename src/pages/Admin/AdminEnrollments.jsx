    ...
    },
    {
      ...
      render: (row) => (
        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
          ...
          row.status === 'Active' ? 'bg-orange-100 text-orange-800 dark:bg-orange-900 dark:text-blue-200' :
      ...
      render: (row) => (
        <div className="flex items-center space-x-2 w-32">
          <div className="flex-1 h-2 bg-gray-200 dark:bg-gray-700 rounded-full">
            <div 
              className={`h-2 rounded-full ${row.progress === 100 ? 'bg-green-600' : 'bg-brand-orange'}`} 
    ...
    <div className="space-y-6">
      ...
      <DataTable 
        ...
        emptyState={{
          ...
          action: (
            <button className="px-4 py-2 bg-brand-orange text-white rounded-md hover:bg-brand-orange-dark transition">
              View Course Catalog
            </button>
          )
        }}
